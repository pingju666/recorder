...
Row {
    ...
    Component.onCompleted: {
        ...
        for (var i = 0; i < videoSourceModel.count; i++) {
            var item = videoSourceModel.get(i)
            if (item.value && item.value.type === 'screen') {
    ...
    ComboBox {
        ...
        displayText: typeof currentValue === 'undefined' ||
                     currentValue.type === 'toggler' ? "不可用" : currentText
